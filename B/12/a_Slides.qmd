...
::: {.fsmaller}
We can use `str_c()` to concatenate two or more vectors of strings.
:::

```{r}
df |> 
  mutate(full_name = str_c(first, " ", last))
```

## String Interpolation

::: {.fsmaller}
**Question:** *Is there an easier way to mix variables and text?*
...
::: {.fsmaller}
The `separate_wider_delim()` function breaks a column apart based on a specific character delimiter.
:::
...
::: {.fsmaller}
**Question:** *What if a messy column contains multiple observations that should be separate rows?*
:::
...
::: {.fsmaller}
The `separate_longer_delim()` function breaks a column apart and creates a new row for each piece. This is essential for creating tidy data!
:::
...
Organizations constantly embed important metadata inside complex ID codes or filenames.
:::

```{r}
df <- 
  tibble(sku = c("US-APP-2023-11", "CA-BAN-2024-01")) |> 
  print()
```
...
::: {.fsmaller}
We can easily break these codes apart to analyze their components, such as finding the country of origin and the product code.
:::
...
:::{.fragment}
```{r}
# Remove outer (starting and ending) whitespace only
str_trim(x)
```
:::
