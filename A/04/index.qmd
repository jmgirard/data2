...
### Slides
```{=html}
<iframe class="slide-deck" src="c_Slides.html" title="[04c] Slideshow">
</iframe>
```
...
Rewrite the following "traditional" code using the native pipe operator.

a) **Nested Math:** Rewrite this calculation to find the square root of 250, and then round the result to one decimal place.

```{r}
# Traditional approach
round(sqrt(250), digits = 1)
```

b) **The Placeholder:** Rewrite this repetition command. We want to repeat the word "Beetlejuice" exactly 3 times. Use the pipe to pass the number **3** into the **times** argument of the `rep()` function.
...
Write a single pipeline to answer the following question about breakfast cereals.

**Question:** Which cereals made by "General Mills" have high sugar content (greater than 10 grams)?

**Requirements:**

1. Start by importing the data from the [cereal.csv](../../data/cereal.csv){download="cereal.csv"} file
1. Filter for the manufacturer (**mfr**) "General Mills" AND **sugars** greater than 10.
1. Select only the **name** and **sugars** columns.
1. Sort the results by **sugars** to place highest sugar at the top.
...
# Note: Ensure cereal.csv is in your project folder
library(tidyverse)
read_csv("../../data/cereal.csv") |> 
  filter(mfr == "General Mills", sugars > 10) |> 
  select(name, sugars) |> 
  arrange(desc(sugars))
```
